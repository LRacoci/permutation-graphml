...
#     input_graphs, target_graphs, graphs = generate_surface_graphs(
#         rand, num_examples, num_nodes_min_max, theta)
...
def generate_networkx_graphs(rand, num_examples, num_nodes_min_max, theta):
    ...
    for _ in range(num_examples):
        ...
        graph = generate_surface_graphs(rand, num_examples=num_examples, min_max_nodes=num_nodes_min_max, geo_density=theta)[0]
...
#     inputs, targets, raw_graphs = generate_surface_graphs(
#         rand, 
#         batch_size, 
#         num_nodes_min_max, 
#         theta
    ...
    def intermediate_solution_nodes(self):
        if self._intermediate_solution_nodes is None:
            self._intermediate_solution_nodes = [
                n for n in self.nodes
                if self._graph.node[n].get("solution", False) 
                    and not self._graph.node[n].get("start", False) 
                    and not self._graph.node[n].get("end", False)
            ]
