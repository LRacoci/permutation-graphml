# -*- coding: utf-8 -*-
"""shortest_path2.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1a8zoq7ic5svHKgMNGq8D1pkWyyLapI6Y
"""
...
def generate_raw_graph(rand, num_nodes_min_max, theta):
    """Generate graphs for training.
...
def generate_raw_graphs(rand, num_examples, min_max_nodes, theta):
    return [generate_raw_graph(rand, min_max_nodes, theta) for _ in range(num_examples)]
...
for j, graph in enumerate(inputs_perm):
  print("permutations: ", inputs_perm[j].graph)
  ax = fig.add_subplot(h, w, j + 1)
  pos = get_node_dict(graph, "pos")
  plotter = GraphPlotter(ax, graph, pos)
  plotter.draw_graph_with_solution()
  
for j, graph in enumerate(targets_perm):
  print("targets_perm: ", targets_perm[j].graph)
  ax = fig.add_subplot(h, w, j + 1)
  pos = get_node_dict(graph, "pos")
  plotter = GraphPlotter(ax, graph, pos)
  plotter.draw_graph_with_solution()
...
for j, graph in enumerate(targets_perm):
  print("targets_perm:", targets_perm)
  ax = fig.add_subplot(h, w, j + 1)
  pos = get_node_dict(graph, "pos")
  plotter = GraphPlotter(ax, graph, pos)
  plotter.draw_graph_with_solution()
...
raw_graphs = generate_raw_graphs(rand, batch_size_tr, num_nodes_min_max_tr, theta)
...
output_ops_ge_perm = model(input_ph, num_processing_steps_ge)
...
loss_ops_ge_perm = create_loss_ops(target_ph, output_ops_ge)
loss_op_ge_perm = loss_ops_ge_perm[-1]  # Loss from final processing step.
...
for iteration in range(last_iteration, num_training_iterations):
  ...
  if elapsed_since_last_log > log_every_seconds:
    ...
    raw_graphs_ge = generate_raw_graphs(rand, batch_size_tr, num_nodes_min_max_ge, theta)
    sources_ge, targets_ge = generate_networkx_graphs(raw_graphs_ge)
    input_ph_ge, target_ph_ge = create_placeholders(raw_graphs_ge)
    
    # A list of outputs, one per processing step.
    output_ops_ge = model(input_ph_ge, num_processing_steps_ge)

    # Test/generalization loss.
    loss_ops_ge = create_loss_ops(target_ph_ge, output_ops_ge)
    loss_op_ge = loss_ops_ge[-1]  # Loss from final processing step.

    # Lets an iterable of TF graphs be output from a session as NP graphs.
    input_ph_ge, target_ph_ge = make_all_runnable_in_session(input_ph_ge, target_ph_ge)
    
    feed_dict_ge = create_feed_dict(sources_ge, targets_ge, input_ph_ge, target_ph_ge)
    test_values_ge = sess.run({
        "target": target_ph_ge,
        "loss": loss_op_ge,
    ...
    perm = [np.random.permutation(len(raw_graphs_ge[i].nodes)) for i in range(len(raw_graphs_ge))]
#     print("raw_graphs: ", raw_graphs[0].nodes)
#     print("raw_graphs: ", raw_graphs[1].nodes)
#     print("perm: ", perm)
    
    #Permute raw_graphs
    raw_graphs_ge_perm = [
        nx.relabel_nodes(raw_graphs_ge[i], mapping={i: p for i,p in enumerate(perm[i])}) 
        for i in range(len(raw_graphs_ge))
    ]
    
#     print("raw_graphs2: ", raw_graphs2[0].nodes)
#     print("raw_graphs2: ", raw_graphs2[1].nodes)

    sources_ge_perm, targets_ge_perm = generate_networkx_graphs(raw_graphs_ge_perm)
    input_ph_ge_perm, target_ph_ge_perm = create_placeholders(raw_graphs_ge_perm)

    # A list of outputs, one per processing step.
    output_ops_ge_perm = model(input_ph_ge_perm, num_processing_steps_ge)

    # Test/generalization loss.
    loss_ops_ge_perm = create_loss_ops(target_ph_ge_perm, output_ops_ge_perm)
    loss_op_ge_perm = loss_ops_ge_perm[-1]  # Loss from final processing step.

    # Lets an iterable of TF graphs be output from a session as NP graphs.
    input_ph_ge_perm, target_ph_ge_perm = make_all_runnable_in_session(input_ph_ge_perm, target_ph_ge_perm)

    feed_dict_ge_perm = create_feed_dict(sources_ge_perm, targets_ge_perm, input_ph_ge_perm, target_ph_ge_perm)
    test_values_ge_perm = sess.run({
        "target": target_ph_ge_perm,
        "loss": loss_op_ge_perm,
        "outputs": output_ops_ge_perm
    },
                           feed_dict=feed_dict_ge_perm)
    correct_tr, solved_tr = compute_accuracy(
        train_values["target"], train_values["outputs"][-1], use_edges=True)
    
    correct_ge, solved_ge = compute_accuracy(
        test_values_ge["target"], test_values_ge["outputs"][-1], use_edges=True)
    
    correct_ge_perm, solved_ge_perm = compute_accuracy(
        test_values_ge_perm["target"], test_values_ge_perm["outputs"][-1], use_edges=True)
    ...
    losses_ge.append(test_values_ge["loss"])
    corrects_ge.append(correct_ge)
    solveds_ge.append(solved_ge)
    
    losses_ge_perm.append(test_values_ge_perm["loss"])
    corrects_ge_perm.append(correct_ge_perm)
    solveds_ge_perm.append(solved_ge_perm)
    
    logged_iterations.append(iteration)
    print("# {:05d}, T {:.1f}, Ltr {:.4f}, Lge {:.4f}, Lpe {:.4f}, Ctr {:.4f}, Str"
          " {:.4f}, Cge {:.4f}, Sge {:.4f}, Cpe {:.4f}, Spe {:.4f}".format(
              iteration, elapsed, train_values["loss"], test_values_ge["loss"],
              test_values_ge_perm["loss"], correct_tr, solved_tr, correct_ge, solved_ge,
              correct_ge_perm, solved_ge_perm))
